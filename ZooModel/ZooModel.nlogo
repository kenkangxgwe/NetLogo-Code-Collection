...
to initial-setting
    ...
    gen-want-to
    set been-to []
    set speed item random 3 [1.2 1.5 1.8]
    set status "going"
    set force-factor 0.5
    select-next 
end
...
to gen-want-to
    ...
    repeat (random total-attractiveness) [
        ...
        set want-to lput index want-to
    ]
    set want-to remove-duplicates want-to
end
...
to get-direction
    if (status = "going" or status = "leaving") [
        set heading towardsxy destx desty 
        let shift 0
        ask other turtles in-cone 5 90 [
            if (xcor = [destx] of myself and ycor = [desty] of myself)
                [stop]
            if (xcor = [xcor] of myself and ycor = [ycor] of myself)
                [stop]
            ifelse (subtract-headings [towards myself] of myself [heading] of myself > 0) [
        ...
        set heading heading + atan shift speed
    ]
end
...
to move
    if (status = "going" or status = "leaving") [
        fd speed
        ifelse (random-float 1 < (xcor - floor xcor)) [
            set xcor floor xcor
        ] [
            set xcor ceiling xcor
        ]
        ifelse (random-float 1 < (ycor - floor ycor)) [
            set ycor floor ycor
        ] [
